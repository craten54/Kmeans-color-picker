...
try:
    ...
    # Asumsi gummy.png adalah PNG. Sesuaikan jika formatnya berbeda.
    ...
    background_image_url = "" # Set kosong jika gambar tidak ditemukan, agar aplikasi tetap jalan
...
st.markdown(f"""
    ...
    /* Ini adalah div yang membungkus sebagian besar konten Streamlit (misalnya .main) */
    .st-emotion-cache-1c7y2vl, .css-1aumxhk {{ /* Ini adalah selector yang seringkali membungkus konten utama */
...
if uploaded_file is not None:
    ...
    # st.columns akan membuat kolom berdampingan
    # Rasio [0.6, 0.4] berarti kolom pertama 60% lebar, kolom kedua 40%
    col_image, col_palette = st.columns([0.6, 0.4]) 

    with col_image:
        st.subheader("📷 Gambar yang Diunggah")
        st.image(image, caption='Gambar Asli', use_container_width=True)
        st.markdown("<br>", unsafe_allow_html=True) # Tambahkan sedikit spasi

    with col_palette:
        st.subheader("🎯 Palet Warna Dominan")
        st.write("🔍 Menganalisis warna dominan...") # Pindahkan ke atas hasil palet
        ...
        # Tampilkan palet warna secara vertikal di dalam kolom palet
